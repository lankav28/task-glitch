...
function AppContent() {
  const {
    ...
    undoDelete,
    lastDeleted,
    resetUndoState, // ✅ newly added helper from context
  } = useTasksContext();
  ...
  // ✅ Fix: clear deleted state when undoing or closing snackbar
  const handleUndo = useCallback(() => {
    undoDelete();
    resetUndoState(); // clear after restoring
    setActivity((prev) =>
      [createActivity("undo", "Undo delete"), ...prev].slice(0, 50)
    );
  }, [undoDelete, resetUndoState, createActivity]);

  const handleCloseUndo = useCallback(() => {
    resetUndoState(); // clear lastDeleted when snackbar closes
  }, [resetUndoState]);
